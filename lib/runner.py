...
def evaluate_agent(agent, world, world_data, agent_index, evaluation_index, data_queue, visualize = None):
    fitness = 0
    ...
    while world_is_alive(world) and fitness < const.MAX_STEPS:
        update_smell(world)
        species_order = torch.randperm(len(const.SPECIES_MAP.keys()), device=device)
        ...
        for species in species_order:
            ...
            selected_set_mask = torch.randint(0, 2, (const.WORLD_SIZE, const.WORLD_SIZE), device=device).bool()
            ...
            biomass_offset = const.SPECIES_MAP_INDEX[species.item()]["biomass_offset"]
            ...
            action_values_batch = agent.forward(batch_tensor, species.item())

            # Step 7: Perform actions
            world = perform_action(world, action_values_batch, species.item(), selected_positions_padded)
...
class Runner():
    def __init__(self, map_folder = 'maps/baltic'):
        ...
        self.world = torch.nn.functional.pad(world, (0, 0, 1, 1, 1, 1), "constant", 0).to(device)
        self.world_data = torch.nn.functional.pad(world_data, (0, 0, 1, 1, 1, 1), "constant", 0).to(device)
        self.generation_finished = threading.Event()

    def simulate(self, agent_file, visualize):
        agent = torch.load(agent_file).to(device)
    ...
    def run(self, single_run=False):
        ...
        for agent_index, (agent, _) in enumerate(self.agents):
            for evaluation_index, _ in enumerate(range(const.AGENT_EVALUATIONS)):
                result = evaluate_agent(agent, self.world, self.world_data, agent_index, evaluation_index, None)
