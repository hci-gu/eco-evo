...
class ARSTrainer:
    ...
    def __init__(self, env_builder, policy_params, sigma=0.1, lr=0.02, n_deltas=8, n_workers=1,
        ...
        # externally by train.py. Each entry is an (impact_seed, spawn_seed)
        # pair describing one independent world that every delta-pair is
        # evaluated against.
    ...
    def _resolve_world_builders(self):
        ...
        - len > 1: builds M sibling builders via
          ``self.env_builder.with_world(impact_seed, spawn_seed)``, one per
          world in ``self.world_list``.
