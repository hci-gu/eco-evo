    ...
    for species, properties in species_map.items():
        ...
        if num_existing < MIN_CELLS_WITH_BIOMASS:
            ...
            for x, y in all_indices:
                ...
                world_array[x, y, energy_offset] = settings.max_energy
                if properties.hardcoded_logic:
                    world_data[x, y, 1] = 1
...
def add_species_to_map_even(settings: Settings, species_map: SpeciesMap, world_array, world_data, seed=None):
    if seed is None:
        seed = int(random.random() * 100000)
    rng = np.random.default_rng(seed)

    world_data[:, :, 4] = 0  # Reset (e.g., plankton cluster marker layer)
    MIN_CELLS_WITH_BIOMASS = int(settings.world_size * settings.world_size * 0.15)
    tiny_world = settings.world_size <= 9
    if tiny_world:
        MIN_CELLS_WITH_BIOMASS = settings.world_size * settings.world_size
    ...
    for species, properties in species_map.items():
        ...
        for x, y in chosen:
            world_array[x, y, biomass_offset] = biomass_per_cell
            world_array[x, y, energy_offset] = settings.max_energy * rng.uniform(0, 1)
            if properties.hardcoded_logic:
                world_data[x, y, 1] = 1
                world_data[x, y, 2] = properties.hardcoded_rules["respawn_delay"]
...
def read_map_from_file(settings: Settings, species_map: SpeciesMap, folder_path, seed=None):
    image = Image.open(folder_path + '/map.png')
    image = image.resize((settings.world_size, settings.world_size), resample=Image.NEAREST)
    pixels = image.load()
    ...
    depth_image = depth_image.resize((settings.world_size, settings.world_size), resample=Image.NEAREST)
    ...
    starting_biomasses = add_species_to_map_even(settings, species_map, world_array, world_data, seed=seed)
