...
def get_movement_delta(world, world_data, species_key, action_values_batch, positions):
    ...
    # movement_deltas = np.zeros((const.WORLD_SIZE + 1, const.WORLD_SIZE + 1, 1), dtype=np.float32)
    ...
    if valid_up_mask.any():
        ...
        movement_deltas[x_batch[valid_up_mask], y_batch[valid_up_mask] - 1, biomass_offset] += biomass_up[valid_up_mask]

    if valid_down_mask.any():
        total_biomass_moved[valid_down_mask] += biomass_down[valid_down_mask]
        movement_deltas[x_batch[valid_down_mask], y_batch[valid_down_mask] + 1, biomass_offset] += biomass_down[valid_down_mask]

    if valid_left_mask.any():
        total_biomass_moved[valid_left_mask] += biomass_left[valid_left_mask]
        movement_deltas[x_batch[valid_left_mask] - 1, y_batch[valid_left_mask], biomass_offset] += biomass_left[valid_left_mask]

    if valid_right_mask.any():
        total_biomass_moved[valid_right_mask] += biomass_right[valid_right_mask]
        movement_deltas[x_batch[valid_right_mask] + 1, y_batch[valid_right_mask], biomass_offset] += biomass_right[valid_right_mask]
...
def perform_eating(world, species_key, action_values_batch, positions):
    ...
    initial_biomass = world[x_batch, y_batch, biomass_offset]
    world[x_batch, y_batch, biomass_offset] -= (initial_biomass * activity_mr_loss)
    total_eat_amount = initial_biomass * eat * const.SPECIES_MAP[species_key]["max_consumption_rate"]
