...
def rebuild_batched_weights(env):
    ...
    if env.N_dm == 0:
        return
    # Batching requires every DM to have an actual network: a missing or
    # explicitly None entry means the env is driven by manual actions.
    if not all(env.policies.get(fid) is not None for fid in env.dm_ids):
        return
...
class PolicyController:
    ...
    def forward(self, observation):
        ...
        if env._batched_ready and obs_np.shape[-1] == env._in_dim:
            ...
            for i, fid in enumerate(env.dm_ids):
                if env.policies.get(fid) is not None:
                    in_dim_i = int(env.per_dm_in_dim[i])
