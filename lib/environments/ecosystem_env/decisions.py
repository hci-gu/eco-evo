...
def accumulate_observation_stats(env, obs_np):
    dim = obs_np.shape[-1]
    if env.N_dm == 0 or dim == 0:
        # NDM-only project: no decision maker owns an observation, so
        # there are no running statistics to merge for the trainer.
        return
...
def update_action_diagnostics(env, probs):
    if env.N_dm == 0:
        # NDM-only project: no actions are taken, so there is nothing to
        # aggregate for the action-mix diagnostics.
        return
