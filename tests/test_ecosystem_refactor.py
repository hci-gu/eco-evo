...
from lib.environments.ecosystem_env.constants import (  # noqa: E402
    MAX_HARVEST_FRAC,
    MOVE_SLICE,
    NORTH,
)
...
def test_predation_respects_current_hide_visibility_floor():
    ...
    # The prey hides completely this tick, but its visibility floor of 0.5
    # keeps half of the 10.0 exposed. Only that visible share is
    # harvestable, and predation may take at most MAX_HARVEST_FRAC of it
    # (Fix 2, Section 73), so the visible part keeps a survivor margin of
    # visible * (1 - MAX_HARVEST_FRAC) instead of being zeroed -- the same
    # contract tests/test_absorbing_zero.py asserts for unhidden prey.
    visible = 5.0
    hidden = 5.0
    expected_intake = visible * float(MAX_HARVEST_FRAC)
    expected_remaining = hidden + visible * (1.0 - float(MAX_HARVEST_FRAC))

    assert env.fgs["prey"].biomass[0, 0] == pytest.approx(expected_remaining)
    assert env.loss_predation["prey"] == pytest.approx(expected_intake)
    assert env.intake_by_pred_prey["pred"]["prey"] == pytest.approx(
        expected_intake)
...
def test_step_without_actions_runs_a_full_tick():
    """``step()`` with no argument must run observation -> policy -> apply.

    This is the monolithic ``step()`` contract the calibrated code relies
    on: the env is self-driving, so ``step()`` builds the observation,
    evaluates the policies itself and publishes the resulting action
    distribution. ``step(actions)`` skips the forward pass and applies
    the supplied distribution instead.
    """
    fgs = {
        "dm": _fg(
            "dm",
    ...
    def _tracking_get_observation():
        calls.append("observation")
        return original_get_observation()

    env.get_observation = _tracking_get_observation

    env.step()

    # The observation was built by step() itself and the action
    # distribution it derived is published under the monolith's names.
    assert calls == ["observation"]
    assert env.tick_count == 1
    assert env.pi_move.shape == (env.N_dm, 4, 1, 1)
    assert env.pi_rest.shape == (env.N_dm, 1, 1)
    assert env.pi_eat.shape == (env.N_dm, env.N_all, 1, 1)
    probability_sum = (
        env.pi_move.sum(axis=1) + env.pi_rest + env.pi_eat.sum(axis=1)
    )
    assert probability_sum == pytest.approx(np.ones((env.N_dm, 1, 1)))

    # Supplying actions applies them directly, without a policy pass.
    del calls[:]
    rest = np.ones((env.N_dm, 1, 1), dtype=np.float32)
    supplied = ActionProbabilities(
        move=np.zeros((env.N_dm, 4, 1, 1), dtype=np.float32),
        rest=rest,
        eat=np.zeros((env.N_dm, env.N_all, 1, 1), dtype=np.float32),
    )
    ...
    assert calls == []
    assert env.tick_count == 2
    assert env.pi_rest == pytest.approx(rest)
