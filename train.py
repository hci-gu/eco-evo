    ...
    for iv in impact_vars:
        ...
        if spec is not None and vmax > vmin:
            ...
            if w is not None:
                ...
                # Colony+jitter already returns the field on a [0, 1]
                # scale where the amplitudes encode relative source
                # strength (per-colony Uniform(amp_min, amp_max)). Bypass
                # the min-max stretch so heterogeneous source strengths
                # survive (otherwise the strongest cell would always be
                # pushed up to vmax).
                bypass_stretch = (
                    spec.mode == "colony"
...
class _EnvBuilder:
    ...
    callable installs *those exact* impact fields (shared per generation).
    When ``None``, fresh maps are sampled per call from the project's
    impact ranges - used only for the initial temp_env probe before the
    training loop starts.

    The builder also carries the optional ``spawn_seed`` that defines one
    deterministic spatial biomass layout. ``with_world(impact_seed,
    spawn_seed)`` produces a fresh sibling builder bound to a different
    world, sharing the same impact_vars / ranges / grid / project.
    ...
    def __init__(self, impact_maps_snapshot=None, grid_size=None,
        ...
        # Keep the raw impact recipe so ``with_world`` can re-sample maps
        # for a different impact_seed without needing access to module
        # globals (which are reset in spawn-workers).
...
def _probe_biomass(trainer, probe_builder, n_ticks, gen, it, jsonl_path,
    ...
    if rnd_builder is not None:
        try:
            ...
            def _make_rnd_env(random_fids):
                ...
                if getattr(trainer, 'local_reward', None) is not None:
                    source_tracking.attach(e, trainer.local_reward)
                out_dim = 5 + e.N_all
        ...
        try:
            ...
            if rnd_env_for_fid:
                ...
                for fid, _re in rnd_env_for_fid.items():
                    ...
                    elif getattr(trainer, 'local_reward', None) is not None:
                        # Per-cell source-tracked reward, accumulated by
                        # the env itself over the probe rollout.
                        _rr = source_tracking.fitness(_re, fid)
                    else:
                        _rr = _log_e_mean
                    if rnd_stability:
    ...
    record = {
        ...
        # by starvation, predation and impact. 'total' is the absolute loss
        # in tons.
...
def main(argv=None, *, on_step=None, confirm=True):
    ...
    elif local_reward is not None:
        _expr = ("log(B/A)" if local_reward.metric == "log" else "B/A")
        print(f"Reward:         local per-cell {_expr}, A=cell energy at tick "
              f"start (user)")
        print(f"                B=end-of-tick energy of that cell's own "
              f"population ({{c,N,E,S,W}})")
        print(f"                {local_reward.norm} over cells, "
              f"theta={local_reward.theta:g}, clip="
              f"[{local_reward.clip_lo:g}, {local_reward.clip_hi:g}], "
              f"mean over ticks")
